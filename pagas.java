...
public class pagas {
    ...
    public double RED20;
    public double RNT;
    public double RNTREDU;
    public double BASE;
    public double REDU;
    public static final double SS = 6.35;
    public static final int MIN_PERSONAL = 5151;
    public static final int[] DESCENDIENTES = { 0, 1836, 3876, 7548, 11730,
            15912 };
    public static final double[][] SITUACION_FAMILIAR = { { 0, 0, 0 },
            { 0, 13662.00, 15617.00 }, { 13335.00, 14774.00, 16952.00 },
            { 11162.00, 11888.00, 12519.00 } };
    public static final double[][] T_IRPF = { { 0, 0 }, { 17707.20, 24.75 },
            { 33007.20, 30 }, { 53407.2, 40 }, { 120000.20, 47 },
            { 175000.20, 49 } };

    public static final double[][] BASE_SS = { { 0, 0 }, { 1045.20, 3230.10 },
            { 867.00, 3230.10 }, { 754.20, 3230.10 }, { 748.20, 3230.10 },
            { 748.20, 3230.10 }, { 748.20, 3230.10 }, { 748.20, 3230.10 }, };
    ...
    public double deduccionSS() {
        ...
        if (BRUTO > BASE_SS[cat][0]) {
            d = (BRUTO < (BASE_SS[cat][1] * 12)) ? (BRUTO * SS / 100)
                    : (BASE_SS[cat][1] * 12 * SS / 100);
        } else {
            d = BASE_SS[cat][0];
        }
        return trunca(d);
    ...
    public int getMIN_PERSONAL() {
        int m = 0;
        m = MIN_PERSONAL + (DESCENDIENTES[min25 + mas25] + min3 * 2244) / 2;
        return m;
    }
    ...
    public double min43() {
        ...
        double limite = art80()
                + (BRUTO - (SITUACION_FAMILIAR[SITUACION][hijos] + reduccionLaboral()))
                * 0.43;
        return limite;
    }
    ...
    public double pagaExtra() {
        double anualIRPF = aPagarIRPF();
        return trunca(BRUTO_MENSUAL - (anualIRPF / N_PAGAS));
        // return trunca(bIRPF() - (bIRPF()*getIRPF()/100) -
        // (BRUTO/N_PAGAS)*SS/100 + (BRUTO/12)*SS/100);
    }
